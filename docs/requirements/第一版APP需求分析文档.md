...
![ChatGPT Image 2026年7月11日 17_08_48](./%E7%AC%AC%E4%B8%80%E7%89%88APP%E9%9C%80%E6%B1%82%E5%88%86%E6%9E%90%E6%96%87%E6%A1%A3.assets/ChatGPT%20Image%202026%E5%B9%B47%E6%9C%8811%E6%97%A5%2017_08_48.png)
...
| `topicType`   | 接口类型                                                     | `cmd`、`remote`、`heartbeat`、`status`、`cmd_ack`、`map` |
...
| `productType`       | string |   是 | 设备类型           | `crawler`                                                    |
| `timestamp`         | string |   是 | 消息生成时间       | RFC3339 毫秒级 UTC 时间                                      |
| `linearSpeedCms`    | number |   是 | 线速度，单位 cm/s  | 取值范围 `[-20, 20]`，前进为正、后退为负，摇杆偏移量决定速度大小 |
| `angularSpeedRadps` | number |   是 | 角速度，单位 rad/s | 取值范围 `[-0.5, 0.5]`，正负方向由 Robot 端统一定义，摇杆左右偏移量决定角速度大小 |
...
| 项目                | 规则                                                         |
| ------------------- | ------------------------------------------------------------ |
| 控制方式            | 360° 虚拟摇杆，根据摇杆方向和偏移量计算线速度、角速度        |
| 启用条件            | Robot 已停止、未急停、设备在线                               |
| 按下前 0.5 秒       | APP 不发送 `remote` 消息                                     |
| 持续按住超过 0.5 秒 | APP 按 20Hz 持续发送，约每 50ms 一次                         |
| 松开摇杆            | 松开摇杆时，APP 先发送一条线速度和角速度均为 0 的 remote 消息，然后停止周期发送。 |
...
Robot 周期性上报 APP 页面需要展示的基础状态和地图定位信息。
...
#### 第二版`device/crawler/crawler_00000001/status`接口预计增加字段：

| 字段             | 类型   | 必填 | 含义         | 开发阶段取值     |
| ---------------- | ------ | ---- | ------------ | ---------------- |
| `currentBlockId` | string | 否   | 当前区域编号 | 例如 `block_001` |
| `currentCellId`  | string | 否   | 当前单元编号 | 例如 `cell_012`  |
| `positionX`      | number | 否   | 地图 X 坐标  | 地图坐标系       |
| `positionY`      | number | 否   | 地图 Y 坐标  | 地图坐标系       |
| `headingDeg`     | number | 否   | 机器人朝向   | `0~360` 度       |

APP 根据连续位置数据保存最近 10 秒轨迹并绘制，不要求 Robot 每次发送完整轨迹数组。
...
```json
{
  ...
  第二版预估增加字段：
  "currentBlockId": "block_001",
  "currentCellId": "cell_012",
  "positionX": 12.35,
  "positionY": 6.80,
  "headingDeg": 90.0
}
...
| `deviceId`      | string |   是 | 设备 ID      | `crawler_00000001`                                           |
| `productType`   | string |   是 | 设备类型     | `crawler`                                                    |
| `timestamp`     | string |   是 | 通知时间     | RFC3339 毫秒级 UTC 时间                                      |
| `mapId`         | string |   是 | 地图唯一 ID  | `map_{region}_{mapVersion}`，region为地区名称，比如China_Shanghai，以{国家}_{城市}命名。 |
| `mapName`       | string |   否 | 地图名称     | `map_{region}_{subRegion}`                                   |
| `mapVersion`    | int    |   是 | 地图版本号   | 从 `1` 开始递增                                              |
...
| 地图归属 | 第一版每台机器人使用独立地图           |
| 地图更新 | 同一 `mapId` 更新时，`mapVersion` 递增 |
...
![image-20260711213208083](./%E7%AC%AC%E4%B8%80%E7%89%88APP%E9%9C%80%E6%B1%82%E5%88%86%E6%9E%90%E6%96%87%E6%A1%A3.assets/image-20260711213208083.png)
...
![image-20260711213305406](./%E7%AC%AC%E4%B8%80%E7%89%88APP%E9%9C%80%E6%B1%82%E5%88%86%E6%9E%90%E6%96%87%E6%A1%A3.assets/image-20260711213305406.png)



### 5.1 页面功能

手动遥控页通过 360° 虚拟摇杆控制机器人线速度和角速度，并实时展示在线状态、工作状态、控制模式和当前速度。

### 5.2 进入条件

进入手动遥控页并启用摇杆前，必须同时满足：
...
→ APP 建立 MQTT 连接
→ 订阅当前设备 heartbeat、status、cmd_ack、map
→ heartbeat 判断在线状态
→ status 更新设备运行状态
→ map 通知 APP 下载并缓存地图
→ APP 发布 cmd 或 remote
